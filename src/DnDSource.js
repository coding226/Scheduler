            ...
            endDrag: (props, monitor, component) => {
                ...
                if(isEvent) {
                    ...
                    if(config.relativeMove) {
                        newStart = localeMoment(new Date(event.start)).add(localeMoment(new Date(newStart)).diff(localeMoment(new Date(initialStart))), 'ms').format(DATETIME_FORMAT);
                    } else {
                        if(viewType !== ViewTypes.Day) {
                            let tmpMoment = localeMoment(new Date(newStart));
                            newStart = localeMoment(new Date(event.start)).year(tmpMoment.year()).month(tmpMoment.month()).date(tmpMoment.date()).format(DATETIME_FORMAT);
                        }
                    }
                    let tmpNewEnd = localeMoment(new Date(newStart)).add(localeMoment(new Date(event.end)).diff(localeMoment(new Date(event.start))), 'ms');
                    // Ricalcolo della data fine per vista quindicina
                    if (viewType == ViewTypes.Fortnight) {
                        if (tmpNewEnd.format("DD") > 10 && tmpNewEnd.format("DD") < 20 && tmpNewEnd.format("DD") != 15) {
                            tmpNewEnd = tmpNewEnd.add(15 - tmpNewEnd.format("DD"), "days");
                        } else if (tmpNewEnd.format("DD") > 20 && tmpNewEnd !== tmpNewEnd.endOf("months")) {
                            tmpNewEnd = tmpNewEnd.endOf("months");
                        } else if (tmpNewEnd.format("DD") < 10) {
                            tmpNewEnd = tmpNewEnd.startOf("months").add(-1, "days");
                        }
                    }   
