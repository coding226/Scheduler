...
class HeaderView extends Component {
    ...
    render() {
        ...
        if(cellUnit === CellUnits.Hour){
        ...
        else if(cellUnit === CellUnits.Fortnight) {
            headerList = headers.map((item, index) => {
                ...
                let pFormattedList = config.forthnightCellHeaderFormat.split('|').map(item => datetime.format(item));
                ...
                const pList = pFormattedList.map((item, index) => (
                    <div key={index}>{item.toString().replace("#16", "Q2").replace("#1", "Q1")}</div>
                ));
