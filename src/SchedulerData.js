...
export default class SchedulerData {
    constructor(date=moment().format(DATE_FORMAT), viewType = ViewTypes.Week,
        ...
        this.cellUnit = viewType === ViewTypes.Day ? CellUnits.Hour : 
                        viewType === ViewTypes.Fortnight ? CellUnits.Fortnight :
                        viewType === ViewTypes.Week2 ? CellUnits.Week :
                        viewType === ViewTypes.Month2 ? CellUnits.Month : CellUnits.Day;
        ...
        if(this.viewType !== viewType) {
            ...
            if(viewType === ViewTypes.Custom || viewType === ViewTypes.Custom1 || viewType === ViewTypes.Custom2) {
                ...
                if(this.viewType < viewType){
                    ...
                    else if(viewType === ViewTypes.Fortnight || viewType === ViewTypes.Week2 || viewType === ViewTypes.Month2) {
                        this.startDate = this.localeMoment(new Date(date)).startOf('month').format(DATE_FORMAT);
                        this.endDate = this.localeMoment(new Date(this.startDate)).add(5, 'months').endOf('month').format(DATE_FORMAT);
                    }
                }
                else{
                    ...
                    let now = this.localeMoment(new Date());
    ...
    moveEvent(event, newSlotId, newSlotName, newStart, newEnd){
        ...
        /*event.end = (this.viewType == ViewTypes.Fortnight ? 
            (localeMoment(eventItem.end).format("DD") > 15 ? 
                localeMoment(eventItem.end).endOf('month').format(DATETIME_FORMAT) : 
                localeMoment(eventItem.end).startOf('month').add(14, "days").format(DATETIME_FORMAT)) 
            : newEnd);*/
        event.end = newEnd;
        event.start = newStart;
        this._attachEvent(event);
                ...
                this.viewType === ViewTypes.Month ? this.config.monthResourceTableWidth : (
                    this.viewType === ViewTypes.Year ? this.config.yearResourceTableWidth : (
                        this.viewType === ViewTypes.Quarter ? this.config.quarterResourceTableWidth : (
                            this.viewType === ViewTypes.Fortnight ? this.config.fortnightTableWidth :
                ...
                this.viewType === ViewTypes.Month ? this.config.monthCellWidth : (
                    this.viewType === ViewTypes.Year ? this.config.yearCellWidth : (
                        this.viewType === ViewTypes.Quarter ? this.config.quarterCellWidth : (
                            this.viewType === ViewTypes.Fortnight ? this.config.fortnightCellWidth  :
    ...
    _resolveDate(num, date = undefined){
        ...
        else if(this.viewType === ViewTypes.Fortnight) {
            this.startDate = date != undefined ? this.localeMoment(new Date(date)).startOf('month').format(DATE_FORMAT)
                : this.localeMoment(new Date(this.startDate)).add(num, 'months').format(DATE_FORMAT);
            this.endDate = this.localeMoment(new Date(this.startDate)).add(5, 'months').endOf('month').format(DATE_FORMAT);
        }
        else if(this.viewType === ViewTypes.Week2) {
            this.startDate = date != undefined ? this.localeMoment(new Date(date)).startOf('month').format(DATE_FORMAT)
                : this.localeMoment(new Date(this.startDate)).add(num, 'weeks').format(DATE_FORMAT);
            this.endDate = this.localeMoment(new Date(this.startDate)).add(6, 'months').add(-1, 'days').format(DATE_FORMAT);
        }
        else if(this.viewType === ViewTypes.Month2) {
            this.startDate = date != undefined ? this.localeMoment(new Date(date)).startOf('month').format(DATE_FORMAT)
                : this.localeMoment(new Date(this.startDate)).add(num, 'months').format(DATE_FORMAT);
            this.endDate = this.localeMoment(new Date(this.startDate)).add(5, 'months').endOf('month').format(DATE_FORMAT);
        }
    ...
    _createHeaders() {
        ...
        if(this.showAgenda){
            ...
            if (this.cellUnit === CellUnits.Fortnight) {
                while (header >= start && header <= end) {
                    //let time = getShortMonth(header) + ' ' + header.Year + ' ' + header.Day === 1 ? 'Q1' : 'Q2';
                    let time = header.format(DATETIME_FORMAT);
                    headers.push({ time: time, nonWorkingTime: false });
                    if (header.format("DD") == 1) {
                        header = header.add(15, 'days');
                    } else {
                        header = header.add(-15, 'days');
                        header = header.add(1, 'months');
                    }
                }
            }
    ...
    _createInitRenderData(isEventPerspective, eventGroups, resources, headers) {
        ...
        while(slotStack.length > 0) {
            ...
            if(currentNode.data.indent > 0) {
                currentNode.data.render = (currentNode.data.render == undefined ? (currentNode.data.fixed ? true : this.config.defaultExpanded) : currentNode.data.render);
            } else {
                currentNode.data.render = true;
            }
            if(currentNode.children.length > 0) {
                currentNode.data.hasChildren = true;
                currentNode.data.expanded = (currentNode.data.fixed ? true : this.config.defaultExpanded);
            }
            ...
            for(i=currentNode.children.length -1; i>=0; i--) {
                ...
                if (currentNode.children[i].data.parentId == currentNode.data.slotId && currentNode.data.fixed) currentNode.children[i].data.render = true;
    ...
    _createRenderData() {
        ...
        // TODO: da capire se serve estrapolarla per 
        this.events.forEach((item) => {
                ...
                resourceEvents.headerItems.forEach((header, index) => {
                    ...
                    if(headerEnd > eventStart && headerStart < eventEnd) {
                        ...
                        if(header.count > resourceEvents.rowMaxCount) {
                            ...
                            let newRowHeight = rowsCount * this.config.eventItemLineHeight + (this.config.creatable && this.config.checkConflict === false && this.config.singleRowEvent === false ? 20 : 2);
        ...
        if(cellMaxEventsCount <= cellMaxEventsCountValue || this.behaviors.getSummaryFunc !== undefined) {
            initRenderData.forEach((resourceEvents) => {
                ...
                if(hasSummary) {
                    let rowsCount = (cellMaxEventsCount <= cellMaxEventsCountValue && resourceEvents.rowMaxCount > cellMaxEventsCount) ? cellMaxEventsCount : resourceEvents.rowMaxCount;
                    let newRowHeight = (rowsCount + 1) * this.config.eventItemLineHeight + (this.config.creatable && this.config.checkConflict === false && this.config.singleRowEvent === false ? 20 : 2);
                    if(newRowHeight > resourceEvents.rowHeight)
                        resourceEvents.rowHeight = newRowHeight;
                }
            });
