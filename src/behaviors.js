...
export const getCustomDate = (schedulerData, num, date = undefined) => {
    ...
    let startDate = num === 0 ? selectDate : 
        schedulerData.localeMoment(new Date(selectDate)).add(2*num, 'days').format(DATE_FORMAT),
        endDate = schedulerData.localeMoment(new Date(startDate)).add(1, 'days').format(DATE_FORMAT),
        cellUnit = CellUnits.Hour;
    if(viewType === ViewTypes.Custom1) {
        let monday = schedulerData.localeMoment(new Date(selectDate)).startOf('week').format(DATE_FORMAT);
        startDate = num === 0 ? monday : schedulerData.localeMoment(new Date(monday)).add(2*num, 'weeks').format(DATE_FORMAT);
        endDate = schedulerData.localeMoment(new Date(startDate)).add(1, 'weeks').endOf('week').format(DATE_FORMAT);
        cellUnit = CellUnits.Day;
    } else if(viewType === ViewTypes.Custom2) {
        let firstDayOfMonth = schedulerData.localeMoment(new Date(selectDate)).startOf('month').format(DATE_FORMAT);
        startDate = num === 0 ? firstDayOfMonth : schedulerData.localeMoment(new Date(firstDayOfMonth)).add(2*num, 'months').format(DATE_FORMAT);
        endDate = schedulerData.localeMoment(new Date(startDate)).add(1, 'months').endOf('month').format(DATE_FORMAT);
        cellUnit = CellUnits.Day;
    }
...
export const getDateLabel = (schedulerData, viewType, startDate, endDate) => {
    ...
    let end = schedulerData.localeMoment(new Date(endDate));
