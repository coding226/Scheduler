...
class AgendaResourceEvents extends Component {
    ...
    render(){
        ...
        resourceEvents.headerItems.forEach((item) => {
            let start = localeMoment(new Date(startDate)).format(DATE_FORMAT),
                end = localeMoment(new Date(endDate)).add(1, 'days').format(DATE_FORMAT),
                headerStart = localeMoment(new Date(item.start)).format(DATE_FORMAT),
                headerEnd = localeMoment(new Date(item.end)).format(DATE_FORMAT);

            if(start === headerStart && end === headerEnd) {
                item.events.forEach((evt) => {
                    let durationStart = localeMoment(new Date(startDate));
                    let durationEnd = localeMoment(new Date(endDate)).add(1, 'days');
                    let eventStart = localeMoment(new Date(evt.eventItem.start));
                    let eventEnd = localeMoment(new Date(evt.eventItem.end));
