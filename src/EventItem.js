...
class EventItem extends Component {
    ...
    stopStartDrag = (ev) => {
        ...
        if (cellUnit === CellUnits.Fortnight) {
            let months = (sign > 0 ? Math.floor(count/2) : Math.ceil(count/2));
            let tmpNewStart = localeMoment(new Date(eventItem.start)).add(months * sign, 'months');
            if (months !== (count/2)) {
                if ((tmpNewStart.format("DD") == 1 && sign > 0) || (tmpNewStart.format("DD") == 16 && sign < 0)) {
                    tmpNewStart = tmpNewStart.add(15 * (sign > 0 ? 1 : -1), 'days');
                } else if (tmpNewStart.format("DD") == 1) {
                    tmpNewStart = tmpNewStart.add(-1, 'months').add(15, 'days');
                } else {
                    tmpNewStart = tmpNewStart.add(-15, 'days').add(1, 'months');
                }
            }
            newStart = tmpNewStart.format(DATETIME_FORMAT);
        } else {
            newStart = localeMoment(new Date(eventItem.start)).add(cellUnit === CellUnits.Hour ? count * config.minuteStep : count, cellUnit === CellUnits.Hour ? 'minutes' : 'days').format(DATETIME_FORMAT);
    ...
    stopEndDrag = (ev) => {
        ...
        if (cellUnit === CellUnits.Fortnight) {
            let months = (sign > 0 ? Math.floor(count/2) : Math.ceil(count/2));
            let tmpNewEnd = localeMoment(new Date(eventItem.end)).add(months, 'months');
            if (tmpNewEnd.format("DD") != 15) tmpNewEnd = tmpNewEnd.endOf('month');

            if (months !== (count/2)) {
                if ((tmpNewEnd.format("DD") == 15 && sign < 0) || (tmpNewEnd.format("DD") != 15 && sign > 0)){
                    tmpNewEnd = tmpNewEnd.add(15 * (sign > 0 ? 1 : -1), 'days');
                } else if (tmpNewEnd.format("DD") == 15) {
                    tmpNewEnd = tmpNewEnd.endOf('month');
                } else {
                    tmpNewEnd = tmpNewEnd.startOf('month').add(15-1, 'days');
                }
            }
            newEnd = tmpNewEnd.format(DATETIME_FORMAT);
        } else {
            newEnd = localeMoment(new Date(eventItem.end)).add(cellUnit === CellUnits.Hour ? count * config.minuteStep : count, cellUnit === CellUnits.Hour ? 'minutes' : 'days').format(DATETIME_FORMAT);
        }
        if(count !== 0 && cellUnit !== CellUnits.Hour && config.displayWeekend === false) {
            if(count > 0) {
                let tempCount = 0, i = 0;
    ...
    render() {
        ...
        let readOnly = false;
        if (!!eventItem.readOnly)
            readOnly = eventItem.readOnly;
...
// #fe8a8a
// boxShadow: 'rgb(170, 170, 170) 2px 3px 4px',         
        let eventItemTemplate = (config.singleRowEvent ? 
            (eventItem.summary ?
                (
                    <div className={roundCls + ' event-item-perc'} key={eventItem.id}
                        style={{height: config.eventItemHeight, background: `linear-gradient(to right, #94aff8 0% ${eventItem.percentualChg}%, #b1aaaa ${eventItem.percentualChg}% ${eventItem.percentualChg + eventItem.percentualUnChg}%, #ffffff ${eventItem.percentualChg + eventItem.percentualUnChg}% 100%)`, border: `solid 2px ${((eventItem.percentualChg + eventItem.percentualUnChg) > 100 ? 'red' : bgColor)}`, color: '#333'}}>
                        <span style={{fontWeight: 'bold', fontSize: 'small', marginLeft: '10px', marginRight: '10px', lineHeight: `${config.eventItemHeight}px` }}>{eventItem.percentualChg + eventItem.percentualUnChg}%</span>
                    </div>
                )
                :
                (
                    <div className={roundCls + ' event-item-perc'} key={eventItem.id}
                        style={{height: config.eventItemHeight, background: `linear-gradient(to right, ${bgColor} ${eventItem.percentual}%, #ffffff ${eventItem.percentual}%)`, border: `solid 2px ${((eventItem.percentualChg + eventItem.percentualUnChg) > 100 ? 'red' : bgColor)}`, color: '#333'}}>
                        <span style={{fontWeight: 'bold', fontSize: 'small', marginLeft: '10px', marginRight: '10px', lineHeight: `${config.eventItemHeight}px` }}>{eventItem.percentual}%</span>
                    </div>
                )
            )
            :
            (eventItem.summary ?
                (
                    <div className={roundCls + ' event-item'} key={eventItem.id}
                        style={{height: config.eventItemHeight, background: `linear-gradient(to right, #94aff8 0% ${eventItem.percentualChg}%, #b1aaaa ${eventItem.percentualChg}% ${eventItem.percentualChg + eventItem.percentualUnChg}%, #ffffff ${eventItem.percentualChg + eventItem.percentualUnChg}% 100%)`, border: `solid 2px ${((eventItem.percentualChg + eventItem.percentualUnChg) > 100 ? 'red' : bgColor)}`, color: '#333'}}>
                        <span style={{fontSize: '10px', marginLeft: '10px', lineHeight: `${config.eventItemHeight}px` }}>{eventTitle}</span><span style={{fontWeight: 'bold', fontSize: 'small', marginLeft: '10px', lineHeight: `${config.eventItemHeight}px`, float: 'right' }}>{eventItem.percentualChg + eventItem.percentualUnChg}%</span>
                    </div>
                )
                :
                (
                    <div className={roundCls + ' event-item'} key={eventItem.id}
                        style={{height: config.eventItemHeight, background: `linear-gradient(to right, ${bgColor} ${eventItem.percentual}%, #ffffff ${eventItem.percentual}%)`, border: `solid 2px ${((eventItem.percentualChg + eventItem.percentualUnChg) > 100 ? 'red' : bgColor)}`, color: '#333'}}>
                        <span style={{fontSize: '10px', marginLeft: '10px', lineHeight: `${config.eventItemHeight}px` }}>{eventTitle}</span><span style={{fontWeight: 'bold', fontSize: 'small', marginLeft: '10px', lineHeight: `${config.eventItemHeight}px`, float: 'right' }}>{eventItem.percentual}%</span>
                    </div>
                )
            )
        );
        ...
        let a = <a className="timeline-event" style={{left: left, width: width, top: top}} onClick={() => { if(!!eventItemClick && !readOnly) eventItemClick(schedulerData, eventItem);}}>
            {eventItemTemplate}
            {startResizeDiv}
            {endResizeDiv}
        </a>
