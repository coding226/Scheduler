    ...
    stopDrag = (ev) => {
        ...
        if(config.checkConflict){
            let start = localeMoment(new Date(startTime)),
                end = localeMoment(new Date(endTime));
                ...
                headerItem.events.forEach((evt, idx) => {
                    if(idx < renderEventsMaxIndex && evt !== undefined && evt.render) {
                        ...
                        if(cellUnit === CellUnits.Hour){
                            ...
                            durationEnd = localeMoment(new Date(endDate)).add(config.dayStopTo + 1, 'hours');
                        }
...
//TODO        let dbg_end = new Date();
//TODO        console.log("Var ResourceEvents (ms): " + (dbg_end.getTime() - dbg_start.getTime()));
